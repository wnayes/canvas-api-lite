...
export function RGBAArrayToCSSColorString(arr: RGBAArray): string {
  if (arr[3] === 255) {
    return (
      "#" +
      arr[0].toString().padStart(2, "0") +
      arr[1].toString().padStart(2, "0") +
      arr[2].toString().padStart(2, "0")
    );
  } else {
...
export function CSSColorStringToRGBAArray(value: string): RGBAArray {
    ...
    if (match) {
      ...
      switch (valueLower.length) {
        ...
        case 5:
          ...
          a =
            valueLower.length === 5
              ? parseInt(valueLower.substring(4, 5).repeat(2), 16)
              : 255;
          break;
        ...
        case 9:
          ...
          a =
            valueLower.length === 9
              ? parseInt(valueLower.substring(7, 9), 16)
              : 255;
          break;
  ...
  throwNotImplemented(`Color value ${value} not supported`);
}
